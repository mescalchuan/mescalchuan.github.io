  ...
  <div class="static-page">
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <el-tag type="success" size="mini">迭部县</el-tag><span>综合数据</span>
      </div>
      <el-table
    ...
    <el-card class="box-card">
      <chart v-if="showChart" id="static-1" color="#e5aa42" title="各乡镇股份经济合作社数量一览图" :x-axis="this.economicNums.map(item => item.x)" :datas="this.economicNums.map(item => item.y)"/>
    </el-card>
    <el-card class="box-card">
      <chart v-if="showChart" id="static-2" color="#52b9c8" title="各乡镇股份经济合作社参股农民数量一览图" :x-axis="this.memberNums.map(item => item.x)" :datas="this.memberNums.map(item => item.y)"/>
    </el-card>
    <el-card class="box-card">
      <chart v-if="showChart" id="static-3" color="#6cbf35" title="各乡镇股份经济合作社规模一览图" :x-axis="this.largeNums.map(item => item.x)" :datas="this.largeNums.map(item => item.y)"/>
    </el-card>
  </div>
</template>
...
import { countryStatistics } from '@/api/statistics'
export default {
  ...
  created() {
    countryStatistics().then(res => {
        ...
        res.data.townshipStatisticsInfoList.map(item => {
          ...
          this.townLists.push({
            ...
            value: item.townshipId
          })
          this.townDatas[0][item.townshipName] = item.registerMemberCount
          this.townDatas[1][item.townshipName] = item.joinStockCount
        })
