        ...
        <el-card class="box-card">
          ...
          <el-table
            ...
            <template slot-scope="scope">
              <el-checkbox v-model="menuIds[scope.$index].checked" @change="handleMenuCheck(scope.$index)">{{ scope.row.menuName }}</el-checkbox>
            </template>
            </el-table-column>
            <!-- <el-table-column
              prop="zip"
  ...
  import Back from '../../components/Back'
    ...
    created() {
      ...
      getAuthList().then(async res => {
        this.tableData = res.data.menuInfos
        if(this.type === 'edit') {
          let detail = {}
          let fd = new FormData()
          ...
          this.tableData.map((item, index) => {
            menuIds.push({
              menuId: item.menuId,
              checked: detail.menus.indexOf(item.menuId) > -1
            })
            // let arr = []
    ...
    methods: {
      ...
      submit() {
        let functionIds = this.menuIds.filter(item => item.checked).map(item => item.menuId)
        let params = {
          ...this.form,
          functionIds
        }
